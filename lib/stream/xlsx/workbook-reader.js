...
class WorkbookReader extends EventEmitter {
  ...
  async *parse(input, options) {
          ...
          if (entry.path.match(/xl\/worksheets\/sheet\d+[.]xml/)) {
            ...
            if (this.sharedStrings && this.workbookRels) {
              ...
              await new Promise((resolve, reject) => {
                const createTmpFile = this.options.createTmpFile || tmp.file;
                const wirteTmpFile = this.options.writeTmpFile || fs.createWriteStream;
                createTmpFile((err, path, fd, tempFileCleanupCallback) => {
                  if (err) {
                    return reject(err);
                  }
                  waitingWorkSheets.push({sheetNo, path, tempFileCleanupCallback});

                  const tempStream = wirteTmpFile(path);
    ...
    for (const {sheetNo, path, tempFileCleanupCallback} of waitingWorkSheets) {
      const readTmpFile = this.options.readTmpFile || fs.createReadStream;
      let fileStream = await readTmpFile(path);
